...
function transferWithSpeedLimit(options: CliOptions) {
  const { source, destination, limit } = options;
  ...
  const chunkSize = Number(limit) * 1024;
...
function init() {
  ...
  transferWithSpeedLimit(options);
}
